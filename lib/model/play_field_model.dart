...
class PlayFieldModel extends ChangeNotifier {
  ...
  void addColumn() {
    this.field_state.add(List.filled(field_state.first.length,
        VirtualPixelModel(false, pixel_key: GlobalKey()), growable: true));
  ...
  void FlushFullLines()
  {
    ...
    if(accumulated_entities[i].length == _width)
    {
      ...
      for(int j = (i-1); j >= 0; j--)
      {
        ...
        for(int k = 0; k < len; k++)
        {
          ...
          active_figure.add([accumulated_entities[j].removeLast(), j]);
        }
      }
    ...
    bool ShiftActiveFigure(shiftDirection dir)
    {
      ...
      for (int i = 0; i < active_figure_buffer.length; i++) {
        ...
        var x = field_state[active_el[_width_index]][active_el[_height_index]]
            .pixel_key.currentState.el;
        var y = field_state[next_el[_width_index]][next_el[_height_index]]
            .pixel_key.currentState.el;
      }
